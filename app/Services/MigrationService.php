     ...
     public function station_migrate(){
        ...
        try{
           ...
           //return $company['v1_id'];
            ...
            $id= $value['v1_id'];
            ///track last update
            $exist= Station::where('company_id', $value['id'])->orderBy('created_at', 'DESC')->get()->first();

            if(count($exist) > 0){
                ///compute max milli second
             $date_time = explode(".", ((array)$exist['created_at'])['date'])[0].'.999';
             $sql = "SELECT * FROM gas_stations where companyid = '".$id."' AND datecreated > '".$date_time."'";
                }else {
                 $sql = "SELECT * FROM gas_stations where companyid = '".$id."'";
                }
           ...
           //return $company['v1_id'];
            ...
            if(count($exist) > 0){
             ...
             $sql = "SELECT * FROM pumps where stationid = '".$id."' AND datecreated > '".$date_time."'";
                 ...
                 $sql = "SELECT * FROM pumps where stationid = '".$id."'";
                }
            ...
            if (mysqli_num_rows($result) > 0 and $result !=false ) {
                ...
                while($row =mysqli_fetch_assoc($result) ){
                    ...
                    //$product 
                     $counter++;
                    Pumps::create(['pump_nozzle_code'=> $row['pump_number'], 'v1_id'=> $row['iPumpsId'], 'brand'=> $row['pump_brand'], 'created_at' => $row ['datecreated'], 'product_id' => $product, 'company_id'=>$value['company_id'], 'station_id'=>  $value['id'], 'serial_number'=> $row['serial_number'], 'fcc_pump_nozzle_id'=> $row['oem_pumpId'] ]); 
        ...
        try{
           ...
           //return $company['v1_id'];
            foreach ($station as $key => $value) {
              $id= $value['v1_id'];
              ///track last update
            $exist= Tanks::where('station_id', $value['id'])->orderBy('created_at', 'DESC')->get()->first();

            if(count($exist) > 0){
                ///compute max milli second
             $date_time = explode(".", ((array)$exist['created_at'])['date'])[0].'.999';
             $sql = "SELECT * FROM tanks where stationid = '".$id."' AND datecreated > '".$date_time."'";
                }else {
                 $sql = "SELECT * FROM tanks where stationid = '".$id."'";
                }
            ...
            if (mysqli_num_rows($result) > 0 and $result !=false ) {
                ...
                while($row =mysqli_fetch_assoc($result) ){
                    ...
                    //$product 
                     $counter++;
                    Tanks::create(['code'=> $row['tank_code'], 'v1_id'=> $row['iTanksId'], 'created_at' => $row ['datecreated'], 'product_id' => $product, 'company_id'=>$value['company_id'], 'station_id'=>  $value['id'], 
        ...
        try{
           ...
           //return $company['v1_id'];
            foreach ($station as $key => $value) {
              $id= $value['v1_id'];
              ///track last update
            $exist= TankGroups::where('station_id', $value['id'])->orderBy('created_at', 'DESC')->get()->first();

            if(count($exist) > 0){
                ///compute max milli second
             $date_time = explode(".", ((array)$exist['created_at'])['date'])[0].'.999';
             $sql = "SELECT * FROM tankgroups where stationid = '".$id."' AND datecreated > '".$date_time."'";
                }else {
                 $sql = "SELECT * FROM tankgroups where stationid = '".$id."'";
                }
            ...
            if (mysqli_num_rows($result) > 0 and $result !=false ) {
                ...
                while($row =mysqli_fetch_assoc($result) ){
                    ...
                    $exist= TankGroups::where('name', $row['tankgroupname'])->where('station_id', $value['id'])->get()->first();                     
                    if(count($exist) == 0){
                        //new tankgroup
                    $new_tg = TankGroups::create(['code'=> $row['tankgroupname'], 'v1_id'=> $row['tankgroupid'], 'created_at' => $row ['datecreated'],'company_id'=>$value['company_id'], 'station_id'=>  $value['id'], 
                    'name' => $row ['tankgroupname'] ]); 

                     Tanks::where('code', $row['tankcode'])->update(['tank_group_id'=> $new_tg['id']]);

                    }else{
                        Tanks::where('code', $row['tankcode'])->update(['tank_group_id'=> $exist['id']]);
                    }
        ...
        try{
           ...
           //return $company['v1_id'];
            foreach ($station as $key => $value) {
              $id= $value['v1_id'];
              ///track last update
            $exist= PumpGroups::where('station_id', $value['id'])->orderBy('created_at', 'DESC')->get()->first();

            if(count($exist) > 0){
                ///compute max milli second
             $date_time = explode(".", ((array)$exist['created_at'])['date'])[0].'.999';
             $sql = "SELECT * FROM pumpgroups where stationid = '".$id."' AND datecreated > '".$date_time."'";
                }else {
                 $sql = "SELECT * FROM pumpgroups where stationid = '".$id."'";
                }
            ...
            if (mysqli_num_rows($result) > 0 and $result !=false ) {
                ...
                while($row =mysqli_fetch_assoc($result) ){
                    ...
                    $exist= PumpGroups::where('name', $row['pumpgroupname'])->where('station_id', $value['id'])->get()->first();                     
                    if(count($exist) == 0){
                        //new tankgroup
                    $new_tg = PumpGroups::create(['code'=> $row['pumpgroupname'], 'v1_id'=> $row['pumpgroupid'], 'created_at' => $row ['datecreated'],'company_id'=>$value['company_id'], 'station_id'=>  $value['id'], 
                    'name' => $row ['pumpgroupname'] ]); 

                     Pumps::where('pump_nozzle_code', $row['pumpcode'])->update(['pump_group_id'=> $new_tg['id']]);

                    }else{
                        Pumps::where('pump_nozzle_code', $row['pumpcode'])->update(['pump_group_id'=> $exist['id']]);
                    }
        ...
        try{
           ...
           //return $company['v1_id'];
            foreach ($station as $key => $value) {
              $id= $value['v1_id'];
              ///track last update
            $exist= PumpGroupToTankGroup::where('station_id', $value['id'])->orderBy('created_at', 'DESC')->get()->first();

            if(count($exist) > 0){
                ///compute max milli second
             $date_time = explode(".", ((array)$exist['created_at'])['date'])[0].'.999';
             $sql = "SELECT * FROM Pump_to_Tank_Mapping where stationid = '".$id."' AND datecreated > '".$date_time."'";
                }else {
                 $sql = "SELECT * FROM Pump_to_Tank_Mapping where stationid = '".$id."'";
                }
            ...
            if (mysqli_num_rows($result) > 0 and $result !=false ) {
                ...
                while($row =mysqli_fetch_assoc($result) ){
                    ...
                    $new_pt = PumpGroupToTankGroup::create(['name'=> $row['ptgroupname'], 'v1_id'=> $row['ptgroupid'], 'created_at' => $row ['datecreated'],'company_id'=>$value['company_id'], 'station_id'=>  $value['id'], 
           ...
           //return $company['v1_id'];
            ...
            if (mysqli_num_rows($result) > 0 and $result !=false ) {
                ...
                while($row =mysqli_fetch_assoc($result) ){
                    ...
                    'nozzle_code' => $value ['pump_nozzle_code'],'open_shift_totalizer_reading'=> $value['open_shift_totalizer_reading'], 'shift_1_totalizer_reading'=> $row['shift_1_totalizer_reading'], 'shift_2_totalizer_reading' => $row ['shift_2_totalizer_reading'],'close_shift_totalizer_reading'=>$row['closing_shift_totalizer_reading'], 'shift_1_cash_collected'=>  $row['shift_1_cash_collected'],'created_by' => $user['id'], 
                    'shift_2_cash_collected' => $row ['shift_2_cash_collected'],'cash_collected'=>$row['cash_collected'], 'ppv'=>  $row['PPV'], 
           ...
           //return $company['v1_id'];
            foreach ($pump as $key => $value) {
              ...
              ///track last update
                 ...
                 $sql = "SELECT * FROM dail_stock_readings where tankid = '".$id."'";
            ...
            if (mysqli_num_rows($result) > 0 and $result !=false ) {
                ...
                while($row =mysqli_fetch_assoc($result) ){
                    ...
                    $new_tg = DailyStockReadings::create(['company_id'=> $value['company_id'], 'v1_id'=> $row['iDailyStockReadingId'], 'station_id' => $value ['station_id'],'tank_id'=>$value['id'], 'status'=>  $row['status'], 
