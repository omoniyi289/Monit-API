...
class StationUserService
{
    ...
    {
        return $this->station_user_repository->get($options);
    }
    ...
    public function create($data)
    {
        ...
        try {
            $station_user = $this->station_user_repository->create($data);
    ...
    public function update($station_user_id, array $data)
    {
        $station_user = $this->get_requested_user($station_user_id);
        $this->database->beginTransaction();
        try {
            $this->station_user_repository->update($station_user, $data);
        } catch (Exception $exception) {
            $this->database->rollBack();
            throw $exception;
        }
    ...
    {
        return $this->station_user_repository->get_where("email", $email);
    }

    public function get_user_by_username($username)
    {
        return $this->station_user_repository->get_where("username", $username);
    }

    private function get_requested_user($user_id, array $options = [])
    {
        return $this->station_user_repository->get_by_id($user_id, $options);
    }
    ...
    public function add_roles($user_id, array $role_ids)
    {
        ...
        $this->station_user_repository->set_role($user, $role_ids);
