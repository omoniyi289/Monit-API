...
class SendCOPSMail implements ShouldQueue
{
    ...
    public function handle(COPSGenerated $event)
    {
            ...
            //send mail
            $users_in_stations_details =  UsersInStations_Details::where('stationid', $data['station_id'])->where('hasaccess', 1)->where('notification_module', 'Commercial Online Pricing Report')->where('NotificationAllowed', 1)->distinct()->get(['firstname','companyname', 'station_name', 'email']);
