...
class SendCOPSMail implements ShouldQueue
{
    ...
    public function handle(COPSGenerated $event)
    {
            ...
            //send mail
            $users_in_stations_details =  UsersInStations_Details::where('stationid', $station_id)->where('hasaccess', 1)->where('notification_module', 'Commercial Online Pricing Report')->where('NotificationAllowed', 1)->distinct()->get(['firstname','companyname', 'station_name', 'email']);
      ...
      foreach ($users_in_stations_details as $value) {
        if($value['email'] !== null){
          if(!isset($grouped_users[$value['email']])){
            if(isset(SendCOPSMail::$final_pdfs[$value['station_name']])){
            $grouped_users[$value['email']] = array();
            $grouped_filenames[$value['email']] = array();
              array_push($grouped_users[$value['email']], ['email' => $value['email'], 'firstname' => $value['firstname'], 'station' => $value['station_name'], 'companyname' => $value['companyname'], 'pdf' => SendCOPSMail::$final_pdfs[$value['station_name']]]);
           }
          }else{    
            if(isset(SendCOPSMail::$final_pdfs[$value['station_name']])){
              array_push($grouped_users[$value['email']], ['email' => $value['email'], 'firstname' => $value['firstname'],'station' => $value['station_name'],'companyname' => $value['companyname'], 'pdf' => SendCOPSMail::$final_pdfs[$value['station_name']]]);
