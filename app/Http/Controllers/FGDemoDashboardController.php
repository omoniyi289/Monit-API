    ...
    if(count($stations) > 0){
            ...
            foreach ($stations as $value) {
             ///calculate run rate for 30 days per station per product
                ...
                $runrate[$station->name] = array();
                $runrate[$station->name]['PMS']=0;
                $runrate[$station->name]['AGO']=0;
                $runrate[$station->name]['DPK']=0;
            ...
            foreach ($tank_data_for_30_days as $key => $tank_reading) {
                $runrate[$station->name][$tank_reading['product']] = $runrate[$value][$tank_reading['product']]
            ...
            foreach ($companies as $value) {
                ...
                foreach ($station as  $inner_value) {
                       ///calculate run rate for 30 days per station per product
                $runrate[$inner_value->name] = array();
                $runrate[$inner_value->name]['PMS']=0;
                $runrate[$inner_value->name]['AGO']=0;
                $runrate[$inner_value->name]['DPK']=0;
            ...
            $query->select('id','code');}))->with(array('station'=>function($query){
            $query->select('id','name');}))->with(array('company'=>function($query){
            $query->select('id','name');}))->get();

            foreach ($tank_data_for_30_days as $key => $tank_reading) {
                $runrate[$inner_value->name][$tank_reading['product']] = $runrate[$inner_value->name][$tank_reading['product']]
                 + (($tank_reading['phy_shift_start_volume_reading'] - $tank_reading['phy_shift_end_volume_reading']) + ($tank_reading['start_delivery'] + $tank_reading['end_delivery']))/count($tank_data_for_30_days);
            }
        ...
        foreach ($tank_data as $key => $value) {
            ...
            if(!isset($con_stocks_at_hand[$value['station']['state']][$value['station']['city']][$value['station']['name']][$product_stamp])){
                $con_stocks_at_hand[$value['station']['state']][$value['station']['city']][$value['station']['name']][$product_stamp]=0;
                 ...
                 $con_stocks_at_hand[$value['station']['state']][$value['station']['city']][$value['station']['name']][$product_stamp]=$con_stocks_at_hand[$value['station']['state']][$value['station']['city']][$value['station']['name']][$product_stamp]   + ($value['start_delivery'] + $value['end_delivery']);
           }
            
        ///replenishement plan stage 2
         if(!isset($total_current_tank_vol[$value['station']['name']][$product_stamp])){
              $total_current_tank_vol[$value['station']['name']][$product_stamp] = 0;
            }
            ///calc current tank vol .per station per product
            $total_current_tank_vol[$value['station']['name']][$product_stamp] = $total_current_tank_vol[$value['station']['name']][$product_stamp] + $value['phy_shift_end_volume_reading'];
         if(!isset($highest_deadstock_level[$value['station']['name']][$product_stamp])){
              $highest_deadstock_level[$value['station']['name']][$product_stamp] = 0;
            }
            ///determine hight deadstock value per station per product
            $highest_deadstock_level[$value['station']['name']][$product_stamp] = $highest_deadstock_level[$value['station']['name']][$product_stamp] < $value['tank']['deadstock'] ? $value['tank']['deadstock'] : $highest_deadstock_level[$value['station']['name']][$product_stamp];

            if(!isset($highest_reorder_level[$value['station']['name']][$product_stamp])){
              $highest_reorder_level[$value['station']['name']][$product_stamp] = 0;
            }
            ///determine hight reorder value per station per product
            $highest_reorder_level[$value['station']['name']][$product_stamp] = $highest_reorder_level[$value['station']['name']][$product_stamp] < $value['tank']['reorder_volume'] ? $value['tank']['reorder_volume'] : $highest_reorder_level[$value['station']['name']][$product_stamp];
